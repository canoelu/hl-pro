...
import { eventBus, StaticKey, useCanvas, useAction } from '@hl/core'
...
// import { useActionState } from '../../state'
// import { copy, decompose } from '../../toolbars'
// import type { ComponentStyle, ContextmenuItem, Vector } from '../../type'
import { copyText, mod360, throttleFrame, stretchedComponents } from '@hl/utils'
import type { ComponentStyle, IVector } from '@hl/utils'
import styles from './index.module.css'
import { ContextmenuItem } from '../../../../plugins/directive'
...
export default defineComponent({
  props: {
    ...
    info: Object as PropType<Material>,
  ...
  setup(props, { slots }) {
    ...
    const deleteComponent = () => {
      canvasState.removeMaterial(props.index!, props.info!.parent)
    }

    const upComponent = () => {
      canvasState.upMaterial(props.index!, props.info!.parent)
    }

    const downComponent = () => {
      canvasState.downMaterial(props.index!, props.info!.parent)
    }

    const topComponent = () => {
      canvasState.topMaterial(props.index!, props.info!.parent)
    }
    ...
    const copyComponentId = () => {
      const id = canvasState.activeMaterial!.id
    ...
    const contextmenus = (_: HTMLDivElement, event: MouseEvent): Optional<ContextmenuItem[]> => {
      ...
      if (canvasState.activeMaterial && canvasState.activeMaterial.id === props.info!.id) {
        event.stopPropagation()
      } else if (!canvasState.activeMaterial && !props.info!.parent) {
        // 如果当前没有选中组件，就选中最底层的组件
        canvasState.activateMaterial(props.info!, props.index!.toString())
        event.stopPropagation()
      } else {
        return
      }
      ...
      return [
        {
          text: '拆分',
          subText: '',
          disable: canvasState.activeMaterial?.component !== 'Group',
          // handler: decompose
        },
        { divider: true },
        {
    ...
    const appendComponent = () => {
      actionState.appendComponent(canvasState.activeMaterial)
    ...
    const handleDragendShape = (e: MouseEvent) => {
      ...
      console.log(props.info?.id, canvasState.activeMaterial)
      if (!canvasState.activeMaterial || props.info!.id !== canvasState.activeMaterial.id) return
      ...
      const move = throttleFrame((moveEvent) => {
        ...
        // // 修改当前组件样式
        canvasState.syncMaterialLocation({ top, left }, props.info!.parent, false)
      })
      const up = () => {
        // 触发元素停止移动事件，用于隐藏标线
        ...
        if (props.info && props.info.parent) {
          canvasState.resizeAutoMaterial(props.info.parent)
        }
        canvasState.saveCavansMaterials()
      }
    ...
    const handleStretchedShape = (point: string, e: MouseEvent) => {
      ...
      if (!(canvasState.activeMaterial && props.info!.id === canvasState.activeMaterial?.id))
        return
      e.stopPropagation()
      e.preventDefault()
      ...
      const move = throttleFrame((moveEvent: MouseEvent) => {
        ...
        canvasState.syncMaterialLocation({ top, left, width, height }, props.info!.parent, false)
      })
    ...
    const handleRotate = (e: MouseEvent) => {
      ...
      const move = throttleFrame((moveEvent: MouseEvent) => {
        ...
        // 修改当前组件样式
        canvasState.syncMaterialLocation({ rotate }, props.info!.parent, false)
      })
    ...
    const getCursor = () => {
      ...
      const rotate: number = mod360(canvasState.activeMaterial?.style?.rotate) // 取余 360
      const result = {}
    ...
    const keyDown = (e: KeyboardEvent): void => {
      ...
      if (props.info && aliasCtrlKey) {
        ...
        switch (e.code) {
          case 'ArrowLeft':
            e.preventDefault()
            canvasState.syncMaterialLocation(
    ...
    watch(
      () => canvasState.activeMaterial,
      (newValue: Material | undefined) => {
    ...
    const contentRender = () => {
      return (
        <>
          ...
          <x-icon
            class={['absolute top--24px left-50% h-20px ', styles.rotation]}
            name="rotation"
            v-show={isActive.value}
            onMousedown={handleRotate}
          />
          <em class='absolute left-0 -top-6' v-show={showEm.value}>
            ({props.defaultStyle!.left},{props.defaultStyle!.top})
          </em>
          {pointRenderData.map((point) => (
            <div
              class={['absolute bg-white border border-solid border-blue-400 w-2 h-2 rounded-full z-10 ml--4px mt--4px', styles.shapePoint, styles[point.direction], styles[rotateClassName.value]]}
              v-show={isActive.value}
