...
<template>
    <div class="flex h-full bg-container text-base_text  overflow-hidden">
        <div class="designer-sidebar flex flex-col  justify-between  border-r-1 border-color ">
            <n-scrollbar class='hl-designer-menu-top'>
                <div class='hl-designer-menu-item  relative cursor-pointer text-base_text hover:text-primary py-8px mb-1 ml-1px'
                    :class="[{ 'text-primary active': item.key === selectedMenu.key }]" v-for='item in menuList'
                    :key='item.key' @click='() => handleSelect(item)'>
            ...
            <div class='hl-designer-menu-bottom'>bottom</div>
        </div>
