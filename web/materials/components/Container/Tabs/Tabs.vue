...
import { DataMode, useProp, useCanvas, useDesigner, getMaterialInstance } from '@hl/core'
import { makeUuid } from '@hl/utils'
import { filterStyle, getComponentStyle, getInnerComponentShapeStyle, Group, Shape, toPercent } from '@hl/designer'
...
watch(
  ...
  () => {
    ...
    for (let i = 0; i < len; i++) {
      if (!props.component.subMaterials[i]) {
        const groupConfig = new GroupComponent(makeUuid())
        if (mode === 'horizontal') {
        ...
        const groupConfig = props.component.subMaterial[i]
        if (mode === 'horizontal') {
...
const getShapeStyle = (item: Material) => {
...
const content = computed<InstanceType<typeof GroupComponent>>(() => {
  return props.component.subMaterials[activeKey.value]
})
...
const handleDrop = async e => {
  ...
  if (componentName) {
    await designer.loadMaterialClazz(componentName)
    const component: Material = getMaterialInstance({ component: componentName })
    if (!component) {
      return
    }
    // const component: Material = new canvasState.components[componentName]()
    ...
    const parentStyle = props.component.subMaterials[activeKey.value].style
    ...
    props.component.subMaterials[activeKey.value].appendChild(component)
  }
}
</script>
