<template>
  <div v-resize="resizeHandler" class="dv-border-box-1">
    ...
    <svg :width="width" :height="height" :key="item" v-for="item in border" :class="`border-item ${item}`">
      <polygon
      ...
      >
        <animate
          attributeName="fill"
          :values="`${propValue.base.color1};${propValue.base.color2};transparent`"
          :dur="`${propValue.base.dur + 1}s`"
          begin="0s"
          repeatCount="indefinite"
        />
      </polygon>
