    ...
    <svg>
      <defs>
        <linearGradient :id="gradientId1" x1="0%" y1="0%" x2="100%" y2="0%">
          <stop v-for="(lc, index) in linearGradients" :key="index" :offset="`${lc[0]}%`" :stop-color="lc[1]" />
        </linearGradient>
...
const gradientId1 = ref<string>(`percent-pond-gradientId1-${makeUuid()}`)
const gradientId2 = ref<string>(`percent-pond-gradientId2-${makeUuid()}`)
...
const handler = event => {
  ...
  if (propValue.data.datatag && item.TagName === propValue.data.datatag) {
    dataValue.value = Number(((Number(item.TagValue) * 100) / Number(propValue.data.maxValue)).toFixed(2))
  }
}
...
const initData = async () => {
  ...
  //     })
  //     if (resp.status === 200) {
  //       dataValue.value = ((Number(resp.data.TagValue) || 0) * 100) / Number(propValue.data.maxValue)
  //     }
  //   } catch (err: any) {
...
const points = computed(() => {
  ...
  const polylineLength =
    ((width.value - (mergedConfig.value.borderWidth + mergedConfig.value.borderGap) * 2) / 100) * dataValue.value
