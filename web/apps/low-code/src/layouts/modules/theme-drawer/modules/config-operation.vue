...
<template>
  <div class="w-full flex justify-between">
    ...
    <n-button type="error" ghost @click="handleReset">{{ $t('theme.configOperation.resetConfig') }}</n-button>
    <div ref="domRef" data-clipboard-target="#themeConfigCopyTarget">
      <n-button type="primary">{{ $t('theme.configOperation.copyConfig') }}</n-button>
    </div>
  </div>
</template>
